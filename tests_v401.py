...
def test_season_titles_dominate_other_gold(tmpdir: Path):
    # Two season championships must outrank a manager with one season title plus
    # a pile of cup/monthly golds. Hall of Fame is a championship hierarchy.
    pd.DataFrame([
        {"season": "A", "winner": "Nordmo", "runner_up": "", "third_place": "", "note": "", "status": "", "source": ""},
        {"season": "B", "winner": "Nordmo", "runner_up": "", "third_place": "", "note": "", "status": "", "source": ""},
        {"season": "C", "winner": "Mats", "runner_up": "", "third_place": "", "note": "", "status": "", "source": ""},
    ]).to_csv(tmpdir / "overall_results.csv", index=False)
    pd.DataFrame([
        {"season": "A", "winner": "Mats", "runner_up": "", "note": "", "status": "", "source": ""},
        {"season": "B", "winner": "Mats", "runner_up": "", "note": "", "status": "", "source": ""},
        {"season": "C", "winner": "Mats", "runner_up": "", "note": "", "status": "", "source": ""},
    ]).to_csv(tmpdir / "cup_results.csv", index=False)
    pd.DataFrame([
        {"manager": "Mats", "monthly_titles": 8, "status": "", "source": ""},
        {"manager": "Nordmo", "monthly_titles": 0, "status": "", "source": ""},
    ]).to_csv(tmpdir / "official_monthly_titles.csv", index=False)
    store = HistoryStore(tmpdir)
    hof = store.hall_of_fame([])
    assert hof.iloc[0]["display_name"] == "Nordmo", hof[["display_name", "league_gold", "cup_gold", "monthly_gold"]]
    assert int(hof.iloc[0]["league_gold"]) == 2
...
def run():
    ...
    with tempfile.TemporaryDirectory() as td:
        test_season_titles_dominate_other_gold(Path(td))
    test_history_weight()
    print("V401 core tests: OK")
