...
class V820CoreTests(unittest.TestCase):
    ...
    def test_player_catalog_exposes_current_premier_league_image_url(self):
        ...
        self.assertIn("/premierleague25/photos/players/110x140/", catalog[10]["image_url_small"])

    def test_v820_home_is_editorial_not_dashboard(self):
        home = (Path(__file__).resolve().parents[1] / "lro_pages" / "home.py").read_text(encoding="utf-8")
        ui = (Path(__file__).resolve().parents[1] / "lro_ui.py").read_text(encoding="utf-8")
        self.assertIn("ui.sports_front", home)
